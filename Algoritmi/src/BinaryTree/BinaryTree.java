...
public class BinaryTree<K, T> extends Dictionary<K, T> {
    private class Nodo {
	public K chiave;
	public T valore;
	...
	if (padre.sinistra != null)
		...
		if (padre.sinistra != null)
		    put(padre.sinistra, eliminato.destra);
		else
	...
	if (ret == null)
		...
		if (comparator.compare(padre.destra.chiave, key) == 0) {
		    ...
		    if (padre.destra != null)
			put(padre.destra, eliminato.destra);
		    else
    ...
    public T ricercaInProfonditaPost(K ric, BinaryTree<K, T> tree) {
	if (tree.root == null)
	    return null;
	return ricercaInProfonditaIn(ric, tree.root);
    }

    private T ricercaInProfonditaPost(K ric, Nodo root) {
	T p = null;
	if (root != null) {
	    if (root.sinistra != null)
		p = ricercaInProfonditaPost(ric, root.sinistra);
	    else
		p = ricercaInProfonditaPost(ric, root.destra);

	    if (comparator.compare(ric, root.chiave) == 0)
		return p = root.valore;
	}

	return p;
    }

    public T ricercaInProfonditaIn(K ric, BinaryTree<K, T> tree) {
	if (tree.root == null)
	    return null;
	return ricercaInProfonditaIn(ric, tree.root);
    }

    private T ricercaInProfonditaIn(K ric, Nodo root) {
	if (root != null) {
	    T val;
	    if (comparator.compare(ric, root.chiave) == 0)
		return root.valore;
	...
	BinaryTree<Integer, String> al = new BinaryTree<Integer, String>(
		(Integer ar1, Integer ar2) -> ar1.compareTo(ar2));
	al.put(0, "lorenzo");
	al.put(1, "andrea");
	al.put(1, "andrea");
	al.put(2, "big");
